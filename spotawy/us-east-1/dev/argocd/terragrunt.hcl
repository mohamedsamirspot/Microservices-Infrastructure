skip = !read_terragrunt_config(find_in_parent_folders("env.hcl")).locals.enable_argocd

terraform {
  source = "${find_in_parent_folders("k8s-tools")}/argocd"
}
...
generate "provider-argocd" {
  path      = "provider-argocd.tf"
