skip = !read_terragrunt_config(find_in_parent_folders("env.hcl")).locals.enable_loki

terraform {
  source = "${find_in_parent_folders("k8s-tools")}/monitoring/loki"
}
...
generate "provider-loki" {
  path      = "provider-loki.tf"
