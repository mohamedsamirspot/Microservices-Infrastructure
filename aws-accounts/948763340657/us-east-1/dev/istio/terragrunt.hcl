skip = !read_terragrunt_config(find_in_parent_folders("env.hcl")).locals.enable_istio

terraform {
  source = "${find_in_parent_folders("k8s-tools")}/istio"
}
...
generate "provider-istio" {
  path      = "provider-istio.tf"
