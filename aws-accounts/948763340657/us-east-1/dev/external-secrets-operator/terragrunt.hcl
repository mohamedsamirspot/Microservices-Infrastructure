skip = !read_terragrunt_config(find_in_parent_folders("env.hcl")).locals.enable_external_secrets_operator

terraform {
  source = "${find_in_parent_folders("k8s-tools")}/external-secrets-operator"
}
...
generate "provider-external-secrets-operator" {
  path      = "provider-external-secrets-operator.tf"
