skip = !read_terragrunt_config(find_in_parent_folders("env.hcl")).locals.enable_grafana_alloy

terraform {
  source = "${find_in_parent_folders("k8s-tools")}/monitoring/grafana-alloy"
}
...
generate "provider-grafana-alloy" {
  path      = "provider-grafana-alloy.tf"
