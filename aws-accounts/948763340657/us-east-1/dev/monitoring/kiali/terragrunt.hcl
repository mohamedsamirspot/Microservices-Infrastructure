skip = !read_terragrunt_config(find_in_parent_folders("env.hcl")).locals.enable_kiali

terraform {
  source = "${find_in_parent_folders("k8s-tools")}/monitoring/kiali"
}
...
generate "provider-kiali" {
  path      = "provider-kiali.tf"
