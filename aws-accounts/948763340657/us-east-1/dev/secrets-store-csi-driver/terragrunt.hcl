skip = !read_terragrunt_config(find_in_parent_folders("env.hcl")).locals.enable_secrets_store_csi_driver

terraform {
  source = "${find_in_parent_folders("k8s-tools")}/secrets-store-csi-driver"
}
...
generate "provider-secrets-store-csi-driver" {
  path      = "provider-secrets-store-csi-driver.tf"
